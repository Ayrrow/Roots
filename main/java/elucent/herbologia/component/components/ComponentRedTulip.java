	...
	public void doEffect(World world, Entity caster, EnumCastType type, double x, double y, double z, double potency, double duration, double size){
		if (type == EnumCastType.SPELL){
			...
			if (targets.size() > 0 && !world.isRemote){
				...
				skeleton.getEntityData().setBoolean("HBMOD_dropItems", false);
		...
		if (type == EnumCastType.INCENSE){
			if (random.nextInt(32-4*(int)potency) == 0 && !world.isRemote){
				switch(random.nextInt(4)){
					case 0: {
						EntityZombie minion = new EntityZombie(world);
						minion.onInitialSpawn(world.getDifficultyForLocation(new BlockPos(x,y,z)), null);
						minion.setDropItemsWhenDead(false);
						minion.getEntityData().setBoolean("HBMOD_dropItems", false);
						minion.setPosition(x+random.nextDouble(), y+2.0, z+random.nextDouble());
						world.spawnEntityInWorld(minion);
					}
					case 1: {
						EntitySkeleton minion = new EntitySkeleton(world);
						minion.onInitialSpawn(world.getDifficultyForLocation(new BlockPos(x,y,z)), null);
						minion.setDropItemsWhenDead(false);
						minion.getEntityData().setBoolean("HBMOD_dropItems", false);
						minion.setPosition(x+random.nextDouble(), y+2.0, z+random.nextDouble());
					...
					case 2: {
						EntitySpider minion = new EntitySpider(world);
						minion.onInitialSpawn(world.getDifficultyForLocation(new BlockPos(x,y,z)), null);
						minion.setDropItemsWhenDead(false);
						minion.getEntityData().setBoolean("HBMOD_dropItems", false);
						minion.setPosition(x+random.nextDouble(), y+2.0, z+random.nextDouble());
					...
					case 3: {
						EntityCreeper minion = new EntityCreeper(world);
						minion.onInitialSpawn(world.getDifficultyForLocation(new BlockPos(x,y,z)), null);
						minion.setDropItemsWhenDead(false);
						minion.getEntityData().setBoolean("HBMOD_dropItems", false);
						minion.setPosition(x+random.nextDouble(), y+2.0, z+random.nextDouble());
